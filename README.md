...
- 系统从“提示词金库”扩展为“提示词金库 + 灵感资料库”。
- 新增内容类型：提示词、灵感记录、网页摘录、待审阅资料、错误内容。
- AI 保存后会自动识别内容类型，并生成适合该类型的 Markdown 卡片。
- 网页端左侧支持按内容类型筛选。
- Chrome 插件弹窗和侧边栏支持选择保存类型，默认 AI 自动识别。
- 右键保存仍保持一步操作，并由后端自动识别类型。
- 网页端支持提示词版本历史查看和恢复。
- 网页端支持自定义分类新增、改名、删除。
- 搜索结果支持多选后批量手动分类、批量 AI 整理、批量归为错误提示词。
- 手动分类优先级高于 AI 自动分类：人工确认后，后续 AI 整理不会覆盖分类。
- 支持关键词搜索和语义相似搜索两种模式。
- 支持最近使用 / 高频提示词筛选。
- 插件插入、复制和网页端复制会记录使用次数。
...
- 邮箱密码注册与登录
- 提示词新增、编辑、删除、搜索、收藏
- 支持提示词、灵感记录、网页摘录、待审阅资料、错误内容
- AI 自动识别内容类型，普通网页文字不会再被强行塞进提示词列表
- AI 自动生成标题、分类、标签、摘要和 Markdown 文档
- 版本管理：编辑前自动保存历史版本，可恢复旧版本
- 分类管理：支持自定义分类新增、改名、删除
- 批量整理：搜索后多选，批量手动分类、AI 整理、归为错误提示词
- 手动分类优先级高于自动分类
- 语义相似搜索
- 最近使用 / 高频提示词
- Markdown / JSON 导入导出
...
```text
点击插件图标 -> 保存类型 -> AI 自动识别 / 提示词 / 灵感记录 / 网页摘录 / 待审阅资料
```
...
```text
手动归类 = 人工确认
人工确认后的分类不会被后续 AI 自动整理覆盖
AI 仍可更新标题、摘要、Markdown 文档和标签
```
...
保存普通网页文字、文章段落、产品资料、学习笔记时，AI 会优先整理为资料卡片，而不是强行当成提示词。

非提示词内容的 Markdown 卡片通常包含：
